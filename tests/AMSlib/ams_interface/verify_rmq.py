...
def verify(
    ...
    assert (
        len(msgs) == num_iterations
    ), f"Received incorrect number of messsages ({len(msgs)}): expected #msgs ({num_iterations})"
    ...
    for i, msg in enumerate(msgs):
        ...
        ), f"Message #{i}: incorrect #elements ({msg.num_element}) vs. expected #elem {num_elements})"
        assert (
            msg.input_dim == num_inputs
        ), f"Message #{i}: incorrect #inputs ({msg.input_dim}) vs. expected #inputs {num_inputs})"
        assert (
            msg.output_dim == num_outputs
        ), f"Message #{i}: incorrect #outputs ({msg.output_dim}) vs. expected #outputs {num_outputs})"
        assert (
            msg.dtype_byte == dtype
        ), f"Message #{i}: incorrect datatype ({msg.dtype_byte} bytes) vs. expected type {dtype} bytes)"
