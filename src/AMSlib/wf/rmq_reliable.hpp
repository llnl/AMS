 ...
 *  
 *  A channel wrapper based on AMQP::Reliable that allows message callbacks to be installed
 ...
 */
#include <amqpcpp.h>
#include <amqpcpp/deferredpublish.h>
...
class AMSReliable : public BASE
{
private:
  // make sure it is a proper channel
  static_assert(std::is_base_of<Tagger, BASE>::value,
                "base should be derived from a confirmed channel.");
...
public:
                           ...
                           int flags = 0)
  {
    return publish(exchange,
                   routingKey,
                   Envelope(message.data(), message.size()),
                   flags);
  }
  DeferredPublish &publish(const std::string_view &exchange,
                           const std::string_view &routingKey,
                           ...
                           int flags = 0)
  {
    return publish(exchange, routingKey, Envelope(message, size), flags);
  }
  DeferredPublish &publish(const std::string_view &exchange,
                           const std::string_view &routingKey,
                           const char *message,
                           int flags = 0)
  {
    return publish(exchange,
                   routingKey,
                   Envelope(message, strlen(message)),
                   flags);
  }
